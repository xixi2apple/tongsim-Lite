...
#include "MujocoMjcfFile.h"
#include "Misc/FileHelper.h"
#include "Misc/ScopeExit.h"

FMjcfFile::FMjcfFile(const FString& InFile, EMjcfConstructMethod::Type ConstructMethod)
	: RootNode(nullptr), bFileLoaded(false)
{
	LoadFile(InFile, ConstructMethod);
}

bool FMjcfFile::LoadFile(const FString& InFile, EMjcfConstructMethod::Type ConstructMethod)
{
	...
	if(ConstructMethod == EMjcfConstructMethod::ConstructFromFile)
	{
...
FString FMjcfFile::GetLastError() const
{
	return ErrorMessage;
}

void FMjcfFile::Clear()
{
	if(bFileLoaded)
	{
...
bool FMjcfFile::IsValid() const
{
	checkSlow(bFileLoaded ? RootNode != nullptr : RootNode == nullptr);
	return bFileLoaded;
}

const FMjcfNode* FMjcfFile::GetRootNode() const
{
	return RootNode;
}

FMjcfNode* FMjcfFile::GetRootNode()
{
	return RootNode;
}

bool FMjcfFile::Save(const FString& Path)
{
	FString Xml = TEXT("<?xml version=\"1.0\" encoding=\"UTF-8\"?>") LINE_TERMINATOR;

	const FMjcfNode* CurrentNode = GetRootNode();
...
void FMjcfFile::PreProcessInput(TArray<FString>& Input)
{
	// Note: This implementation is written simply and will not handle all cases.  It
	//       is made for the simple cases where FMjcfFile is to be used.
	// Assumptions/Misc:
...
void FMjcfFile::WhiteOut(TArray<FString>& Input, int32 LineStart, int32 LineEnd, int32 IndexStart, int32 IndexEnd)
{
...
void FMjcfFile::Tokenize(FStringView Input, TArray<FString>& Tokens)
{
...
TArray<FString> FMjcfFile::Tokenize(const TArray<FString>& Input)
{
...
static void AddAttribute(const FString& InToken, TArray<FMjcfAttribute>& OutAttributes)
{
	...
	if(InToken.FindChar(TEXT('='), EqualsIdx))
	{
		...
		if(bQuotesRemoved)
		{
			...
			OutAttributes.Add(FMjcfAttribute(InToken.Left(EqualsIdx), Value));
		}
	}
}

enum class EMjcfTryParseTagResult
{
	Error,
	None,
	ParsedStartTag,         // <key>
...
static EMjcfTryParseTagResult TryParseTag(FString& OutTag, TArray<FMjcfAttribute>& Attributes, const FString*& InOutToken, const FString* LastToken)
{
	const FString* Token = InOutToken;

	if (Token == LastToken)
	{
		return EMjcfTryParseTagResult::None;
	}

	if (*Token == TEXT(">") || *Token == TEXT("/>"))
	{
		// Error: Found an unexpected end-of-tag (ex: >list>)
		return EMjcfTryParseTagResult::Error;
	}
	...
	if (*Token == TEXT("</"))
	{
		...
		for (;;)
		{
			...
			{
				InOutToken = Token;
				return EMjcfTryParseTagResult::ParsedEndTag;
			}
	...
	if (*Token != TEXT("<"))
	{
		...
		InOutToken = Token;
		return EMjcfTryParseTagResult::NotATag;
	}
	...
	for (;;)
	{
		...
		{
			InOutToken = Token;
			return EMjcfTryParseTagResult::ParsedEmptyElementTag;
		}
		...
		{
			InOutToken = Token;
			return EMjcfTryParseTagResult::ParsedStartTag;
		}
...
FMjcfNode* FMjcfFile::CreateRootNode(TArrayView<const FString> Tokens)
{
	// Algorithm (Draft):
	...
	TArray<FMjcfNode*> ParsingNodeStack;
	ON_SCOPE_EXIT
	{
	...
	for (;;)
	{
		...
		EMjcfTryParseTagResult ParseResult = TryParseTag(Tag, Attributes, Token, LastToken);
		if (ParseResult == EMjcfTryParseTagResult::None)
		{
		...
		if (ParseResult == EMjcfTryParseTagResult::Error)
		{
			// Malformed tag
		...
		if (ParseResult == EMjcfTryParseTagResult::ParsedEndTag)
		{
			...
			FMjcfNode* FullyParsedNode = ParsingNodeStack.Pop();
		...
		if (ParseResult == EMjcfTryParseTagResult::NotATag)
		{
		...
		check(ParseResult == EMjcfTryParseTagResult::ParsedStartTag || ParseResult == EMjcfTryParseTagResult::ParsedEmptyElementTag);

		FMjcfNode* NewNode = new FMjcfNode();
		...
		else if (ParseResult == EMjcfTryParseTagResult::ParsedEmptyElementTag)
		{
			// If we just parsed an empty element root node, stop
			return NewNode;
		}

		if (ParseResult == EMjcfTryParseTagResult::ParsedStartTag)
		{
...
void FMjcfFile::HookUpNextPtrs(FMjcfNode* Node)
{
	if(Node == nullptr)
	{
		return;
...
void FMjcfFile::WriteNodeHierarchy(const FMjcfNode& Node, const FString& Indent, FString& Output)
{
	// Write the tag
	Output += Indent + FString::Printf(TEXT("<%s"), *Node.GetTag());
	for(const FMjcfAttribute& Attribute: Node.GetAttributes())
	{
	...
	const FMjcfNode* FirstChildNode = Node.GetFirstChildNode();
	...
	{
		Output += TEXT(">") LINE_TERMINATOR;
		for(const FMjcfNode* ChildNode = FirstChildNode; ChildNode != nullptr; ChildNode = ChildNode->GetNextNode())
		{
